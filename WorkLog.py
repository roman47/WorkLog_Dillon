...
def create_new_entry():
    ...
    # be able to provide a task name, a number of
...
def overwrite_work_log(new_entry_information):
    with open(WORK_LOG_FILE, 'w') as f:
        ...
        for entry in new_entry_information:
            writer.writerow([entry["date"], entry["task_title"],
                             str(entry["minutes"]), entry["notes"]])
...
def load_work_log(search_type):
    ...
    if search_type == 'd':
        ...
        for log in all_work_logs:
            log["show_log"] = 1
            if log["task_title"] != search_exact and log["notes"] \
                    != search_exact:
                log["show_log"] = 0
        # for log in all_work_logs:
